...
/**
 * E2E WORKFLOW — B2B SAMPLE (ON THE ORDER) / INHOUSE PRODUCTION.
 *
 * Reworked 10-09-2026 to the DUAL-STREAM flow (same shape as the
 * used-in-production twin, WITHOUT the Used In Production toggle and
 * WITHOUT Used Sample Weight consumption): the B2B order with a sample
 * drives a JOB WORK stream (raised from the order) AND a SAMPLE stream.
 * Both are assigned DIRECTLY to Casting Process / Casting Inspection
 * (one round - no Design/CAD, no process transfer) and go through
 * Process Movement Accept -> Worker Issue -> Worker Receipt each. The
 * JOB WORK worker receipt is a SETTLEMENT item-form (Production No +
 * item details + Move to Job Finalize) and MUST submit BEFORE the
 * SAMPLE worker receipt (which checks Finalize Sample). Then Sample
 * Receipt + Sample Delivery close the flow.
 *
 * Step order:
 ...
 *
 * Twin of b2b-sample-inhouse-usedinprod-workflow - only the Used In
 * Production toggle (order sample panel + sample issue) and the Used
 * Sample Weight consumption on the settlement receipt differ.
 * Grids key rows by the SAMPLE NO / JOB WORK NO captured in state.
 * State: e2e-b2b-sample-state.json.
 *
...
test.describe('B2B Sample - Inhouse - Production - Workflow', () => {
  test('TC-B2B-SMP-01 create the B2B order with a sample', async ({ loginPage, b2bOrderBooking, sampleWorkflow, page }) => {
  ...
  test('TC-B2B-SMP-02 procurement job work inhouse against the order', async ({ loginPage, production, page }) => {
    test.setTimeout(600_000);
    const { orderNo } = state.readState();
    expect(orderNo, 'run TC-B2B-SMP-01 first').toBeTruthy();
    await login(loginPage, page);
  ...
  test('TC-B2B-SMP-03 sample issue inhouse (Procurement > Issue, Sample tab)', async ({ loginPage, sampleWorkflow, page }) => {
    test.setTimeout(600_000);
    const { sampleNo } = state.readState();
    expect(sampleNo, 'run TC-B2B-SMP-01 first').toBeTruthy();
    await login(loginPage, page);
  ...
  test('TC-B2B-SMP-04 assign the JOB WORK to Casting Process / Casting Inspection', async ({ loginPage, production, page }) => {
    test.setTimeout(420_000);
    expect(jobWorkKey(), 'run TC-B2B-SMP-02 first').toBeTruthy();
    await login(loginPage, page);
  ...
  test('TC-B2B-SMP-05 assign the SAMPLE to Casting Process / Casting Inspection', async ({ loginPage, production, page }) => {
    test.setTimeout(420_000);
    expect(rowKey(), 'run TC-B2B-SMP-01 first').toBeTruthy();
    await login(loginPage, page);
    await production.assignJob({
      sourceType: 'Sample',
      itemType: 'Metal',
  ...
  test('TC-B2B-SMP-06 process movement accept - JOB WORK (Casting)', async ({ loginPage, production, page }) => {
    test.setTimeout(420_000);
    await login(loginPage, page);
  ...
  test('TC-B2B-SMP-07 process movement accept - SAMPLE (Casting)', async ({ loginPage, production, page }) => {
    test.setTimeout(420_000);
    await login(loginPage, page);
    await production.processMovementAccept({
      process: DATA.round.process,
  ...
  test('TC-B2B-SMP-08 worker issue - JOB WORK (Casting)', async ({ loginPage, production, page }) => {
    test.setTimeout(600_000);
    await login(loginPage, page);
  ...
  test('TC-B2B-SMP-09 worker issue - SAMPLE (Casting)', async ({ loginPage, production, page }) => {
    test.setTimeout(600_000);
    await login(loginPage, page);
  ...
  test('TC-B2B-SMP-10 worker receipt - JOB WORK settlement (Move to Job Finalize)', async ({ loginPage, production, page }) => {
    test.setTimeout(600_000);
    await login(loginPage, page);
    ...
    // sample receipt. NO Used Sample Weight here - that consumption belongs to
    // the used-in-production twin only.
  ...
  test('TC-B2B-SMP-11 worker receipt - SAMPLE (Finalize Sample)', async ({ loginPage, production, page }) => {
    test.setTimeout(600_000);
    await login(loginPage, page);
    const result = await production.workerReceipt({
      ...DATA.round,
  ...
  test('TC-B2B-SMP-12 sample receipt (Repair page, Sample tab, inhouse)', async ({ loginPage, sampleWorkflow, page }) => {
    test.setTimeout(600_000);
    const { sampleNo } = state.readState();
    expect(sampleNo, 'run TC-B2B-SMP-01 first').toBeTruthy();
    await login(loginPage, page);
  ...
  test('TC-B2B-SMP-13 sample delivery to the customer', async ({ loginPage, sampleWorkflow, page }) => {
    test.setTimeout(600_000);
    const { sampleNo } = state.readState();
    expect(sampleNo, 'run TC-B2B-SMP-01 first').toBeTruthy();
    await login(loginPage, page);
