...
| Production E2E workflow (tests/e2e/production-concept-workflow, TC-PRD-E2E-01…08) | **All 8 steps passing headed** (concept master data moved on 21-09-2026: Concept For "Imit", setting type "Met Stone Setting 5" — both specs updated) — Concept (create/upload/approve) → Job Work → Assignment → Movement Accept → Worker Issue/Receipt (CAD) → Transfer+Accept (Casting) → Issue/Receipt with item → Job Finalize + barcode. Integration chain: document numbers flow via e2e-production-state.json; every step resumes/skips what's already done |
| Production - Concept - CAD - Material workflow (tests/e2e/production-cad-material-workflow, TC-CADM-01…15) | **All 15 steps passing headed (22-09-2026, order per the QA lead's recording)** — the concept chain split into its 3 concept steps (create / upload / approval) → Job Work → Job Assignment (captures the J-series production no) → Process Movement Accept (CAD) → Worker Issue (CAD, Prabhat) → **CAD Upload** while the job is with the CAD worker (Production > Planning > CAD, Upload tab: Worker + Production No J###.1 + 3D Volume + Approx Weight + image) → **CAD Approval** (Approval tab: image card checkbox + Status "Approve") → Worker Receipt (CAD) → Transfer to Casting + Accept → Worker Issue (Casting, **the newest Sioniquser<N>** - read from sioniquser-counter.json as the masters suite's last completed iteration, CASTING_WORKER=<name> overrides; `npm run test:cadm` runs tests/masters and then this chain so every run exercises the employee created minutes earlier) → **Material Issue** (Production > Operations > Material Transaction, Issue tab: employee **Asmi** whose locker "Conter Ab" holds Gold,Ring-Tendulkar 91.60 stock; Metal / **Stock**; tick the stock row - preferred Met Stone Setting 5 / Gold,Ring-Tendulkar / 91.60, falling back to the same article under any metal type, then the same metal type, then any 91.60 Gold row, largest balance first, because the locker rows drift between runs (22-09-2026: the preferred row was used up) → Add opens the **"Metal — Configure" dialog**: Assign Type **Production** + Production No + Gross Weight 50 g, capped to the stock row's remaining balance → Add; the issued row's metal type + article are kept in the state file) → **Material Receipt** 5 g (Receipt tab: same header + Stock Identity Type; the pending grid aggregates the worker's open issues per purity + metal type, so the row is keyed by 91.6 + the issued metal type → Add opens the **"Metal Detail Entry" dialog**: receipt purity 91.60, metal type + article of the ISSUED row so the metal returns to the same stock row, other mandatory selects take their first option, received weight → **Add to Grid**) → Worker Receipt with item 40.000 g (Move to Job Finalize). Material moves exist ONLY at Casting (QA lead). Own state file e2e-cad-material-state.json; steps 06+ refuse to run on an empty state (the grid helpers would otherwise act on somebody else's job). Test cases: test-cases/QA_Production_Concept_CAD_Material_Test_Cases_Industry_Standard.xlsx (TC-E2E-CADM-001, 150 steps) |
...
| Metal Inward - Process with Barcode or Lot = Yes (tests/e2e/metal-inward-barcode-lot-yes-workflow, TC-MIY-01…02) | **Both passing headed (24-09-2026)** — the new post-Submit prompt answered **Yes** opens the "Lot / Barcode" dialog (one select, "Lot or Barcode": Barcode | Lot). **Lot**: a "Lot Employee" select appears (Ubaid); the dialog's Submit saves the inward AND generates its lot in ONE call (the save body carries both numbers, e.g. M328 + NNN15). **Barcode**: one tag card per piece (Employee = login user, Gross Weight prefilled, optional OMS / Certificate / RFID / HUID / Remarks); Submit saves the inward, an auto-generated lot AND its tag(s) in one call (e.g. M329 + NNN16 + 2026-06-2300019). StockInwardBasePage.submitWithPostProcess({ mode, lotEmployee }) drives it; every other Metal Inward chain keeps answering No. Standalone tests, state file e2e-inward-yes-state.json |
| Metal Inward GRN - GRN Sales Invoice workflow (tests/e2e/metal-inward-grn-to-grn-sales-invoice-workflow, TC-GRN-01…06) | **All 6 steps passing headed (24-09-2026, first run)** — goods come in UNPRICED on a GRN inward and go out on a GRN-type B2B sales invoice: Metal Inward (Sub Transaction Type **GRN** first - it drives the vendor fetch; #invoiceNo is the "Voucher Number"; Stock / Direct, vendor **Luxurio**, purchaser Ajin G, 1 pc Tendulkar 91.60, 60 g, no item rate; "Process with Barcode or Lot?" → No) → Lot (employee Ubaid) → Barcode as suja (Stock) → Counter Allocation by LOT (vendor Luxurio, RFID kept) → Counter Accept (RFID row) → **B2B Metal Sales Invoice with Transaction Sub Type "GRN"** (customer RAJA + branch, Counter / Tag Wise, RFID scan; SSS##; b2bSalesInvoice({ subType: "GRN" })). Own state file e2e-grn-sales-state.json |
| Brand Inward - Sales Invoice workflow (tests/not-implemented/brand-inward-sales-invoice-workflow, TC-BSI-01…06) | **NOT IMPLEMENTED - parked out of every run** (steps 01-05 passed headed 24-09-2026; 06 blocked on the app) — Brand Inward (RAJA/Direct/Cochin, Gold/Ring/Amraa/Tendulkar, 10 pcs / 20 g / MRP 90000, no discount) → Lot (Item Type Brand, From Transaction Type Brand Inward) → Barcode as suja (Brand Name + Pricing Amount; a brand tag takes the WHOLE lot - one tag, 10 pcs; tag series 26-06-23####) → Counter Allocation by LOT (Item Type Brand - no Group Category select; the lot fetch names the tag and its RFID) → Counter Accept → **B2B sales invoice: no screen sells Brand stock** - /sls/app-invoice-setup has only the "Metal Invoice" tab and it answers "Tag not found" for the brand tag by RFID and tag number; Retail Operations has no sale screen. Open question for the QA lead. Own state file e2e-brand-sales-state.json |
| Stone Inward - Sales Invoice workflow (tests/not-implemented/stone-inward-sales-invoice-workflow, TC-SSI-01…06) | **NOT IMPLEMENTED - parked out of every run** (steps 01-05 passed headed 24-09-2026; 06 blocked on the app) — Stone Inward (RAJA/Direct, Jerald, Without Tare, Gram, **4 pcs / 6 g** - the "Stone Rule" approval rule holds any Stone Inward with Invoice Amount > 2,000,000 for user Prabhat and Lot Generation lists only APPROVED inwards; the old 5 pcs / 10 g recipe came to 2,260,000; return 5 %, **Assorted Stock** ticked) → Lot (Item Type Stone, From Transaction Type Stone Inward) → Barcode as suja (tag series RS#####) → Counter Allocation by LOT (Item Type Stone; RFID kept) → Counter Accept → **B2B sales invoice: "Tag not found"** - the only invoice screen is the "Metal Invoice" and it does not sell stone (same as Brand). Open question for the QA lead. Own state file e2e-stone-sales-state.json |
| Approval Issue - Approval Receipt - B2B Sales workflow (tests/e2e/approval-issue-receipt-sales-workflow, TC-AIR-01…08) | **All 8 steps passing headed (24-09-2026)** — TWO pieces go out on one approval, one comes back, the other is invoiced: Metal Inward (Stock/Direct, RAJA, **2 pcs** Tendulkar 91.60; pure rate 15000 after Add Item; "Process with Barcode or Lot?" → No) → Lot → Barcode as suja (**one save per piece** - a single save with pieces 2 makes ONE tag carrying 2 pieces) → **Counter Allocation by LOT** (Scan Type "Lot Number": Vendor + Lot Number multi-selects, virtual-scrolled list whose search filters nothing, then "Fetch Items"; the fetch answer's **RFIDs** are kept because TAG NUMBERS REPEAT on qa - the day series restarts under the pinned business date, and a tag-number scan once allocated a stranger's 122 g item) → Counter Accept (both RFID rows) → **Approval Issue** (/sls/view-b2b-approval-issue: Issue From Stock + Issue To Customer reveal Customer + **Customer Branch** (gates the tag lookup), Purpose Display, Salesman/Helper/Supervisor mandatory, Stock Source Counter, Issue Type Tag Wise, Scan Type **Rfid Number**, Enter stages each tag, then the "Metal weight" **Rate (₹/gm)** must be entered or Submit only flashes "Please enter the metal rate"; RC no LLLL##) → **Approval Receipt** of tag A (/sls/view-approval-receipt: Direct / Customer / Stock, Customer lists only customers holding approval stock, "Against RC No(s)" typeahead, Scan By Rfid Number, Received Tags : 1; RRRR##) → **Metal Invoice for tag B with Issue Type "Approval RC No"** (the RC select lists only approvals whose tags are still OUT at the customer; picking it loads the issued tags, tick tag B, "Add Selected" moves it into the invoice grid, metal rate, Submit; SSSS##; a "no active tax template" toast does not block the save). Why not receipt-then-invoice on ONE tag: a received tag is parked in the Default Stock Accept Counter - Counter Allocation, Tag Wise invoice and the RC list all refuse it (Counter Transfer "Return to Counter" exists as returnToCounter but the QA lead does not want it in the flow). Own state file e2e-approval-sales-state.json |
...
| `RUN_NOT_IMPLEMENTED=1 npx playwright test tests/not-implemented --project=no-auth` | Run the parked workflows the app cannot complete yet (Brand / Stone sales invoice); every normal run ignores tests/not-implemented |
| `npm run test:cadm` | Masters suite (new Sioniquser<N>: employee, user, smith, locker, mapping) and then the Concept-CAD-Material chain, which issues to that new worker |
...
- **App changes seen on 23-09-2026 (evening deploy):** the Metal Inward ITEM form lost its per-item "Rate" input - after Add Item a per-metal strip ("Pure Weight" / "Rate" / "Metal Amount") renders and Next / Submit stay silent while that Rate is empty (StockInwardBasePage.addItem enters the PURE rate there - 15000 per the QA lead, 24-09-2026, see PURE_RATE); since 24-09 Metal Inward's Submit first asks "Process with Barcode or Lot?" and the wizards answer **No** (lot and barcode stay separate steps); the Lot Generation item panel lost its Business Unit select (location filter change) and gained more env-configured description dropdowns (LotGenerationPage picks Business Unit only when present and fills empty mandatory selects); the B2B invoice tab is named "Metal Invoice" and its scan commits on Enter; Approval Issue / Receipt scans need the Customer Branch and the "Metal weight" Rate (₹/gm). Barcode TAG NUMBERS REPEAT on qa (day series under the pinned business date 23/06/2026) - key tag-wise screens by lot or RFID (see the Approval chain row).
