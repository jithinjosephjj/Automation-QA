...
| Approval Issue - Approval Receipt - B2B Sales workflow (tests/e2e/approval-issue-receipt-sales-workflow, TC-AIR-01…10) | **Steps 01-08 passing headed (23-09-2026); 09-10 blocked on the app** — Metal Inward (Stock/Direct, RAJA, 1 pc Tendulkar 91.60) → Lot → Barcode (as suja) → **Counter Allocation by LOT** (Scan Type "Lot Number": Vendor + Lot Number multi-selects, the lot list is virtual-scrolled and its search filters nothing, then "Fetch Items"; the scan answer's **RFID** is kept because TAG NUMBERS REPEAT on qa - the day series restarts under the pinned business date, and a tag-number scan once allocated a stranger's 122 g item) → Counter Accept (RFID row) → **Approval Issue** (/sls/view-b2b-approval-issue: Issue From Stock + Issue To Customer reveal Customer + **Customer Branch** (gates the tag lookup), Purpose Display, Salesman/Helper/Supervisor mandatory, Stock Source Counter, Issue Type Tag Wise, Scan Type **Rfid Number**, Enter stages the tag, then the "Metal weight" **Rate (₹/gm)** must be entered or Submit only flashes "Please enter the metal rate"; RC no LLLL##) → **Approval Receipt** (/sls/view-approval-receipt: Direct / Customer / Stock, Customer lists only customers holding approval stock, "Against RC No(s)" typeahead, Scan By Rfid Number receives the tag, Received Tags : 1; RRRR##) → **Counter Transfer "Return to Counter"** (the received tag is parked in the Default Stock Accept Counter "Conter Ab", where Counter Allocation says "Tag / RFID not found or not eligible"; CC## moves it to its counter) → Counter Accept (nothing pending - the transfer lands approved) → **B2B Metal Invoice still refuses the returned tag ("This tag is not in Counter stock.")** - how approval-returned stock is meant to reach the Metal Invoice (which sells from "Counter" only; Issue Type "Approval RC No" bills tags still OUT on approval) is an open question for the QA lead. Invoice tab renamed "Metal Invoice"; its scan commits on Enter. Own state file e2e-approval-sales-state.json |
